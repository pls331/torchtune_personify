...
class UserPrefixArch(nn.Module):
    def __init__(
        ...
        user_token_emb_dim = self.ffn_dim  # same as model's embedding dim
        ...
        self.total_emb_dim: int = self.n_user_token * user_token_emb_dim
    ...
    def forward(self, user_idxs: torch.Tensor) -> torch.Tensor:
        user_prefix_emb = self.user_prefix_emb(user_idxs)  # [B, n_user_token, dim]
        B = user_idxs.shape[0]
        ...
        assert out.shape == (B, self.n_user_token, self.ffn_dim)
        return out